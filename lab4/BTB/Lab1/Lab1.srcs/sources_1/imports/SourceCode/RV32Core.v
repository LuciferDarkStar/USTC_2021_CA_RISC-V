...
module RV32Core(
	...
	BTB BTB1(
		...
		.rd_predicted(BranchPredictedF), //是否预测标志
		.wr_req(BranchPredictedE ^ BranchE), //先前预测和实际跳转情况不同，则需要更新BTB
		.wr_PC(PCE), //wr_req为1 且 实际跳转时才使用
		.wr_predicted_PC(BrNPC), //wr_req为1 且 实际跳转时才使用
		.wr_predicted_state_bit(BranchE) //wr_req为1时，若实际跳转，写入1，否则，写入0
	);
